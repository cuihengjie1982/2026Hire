...
const taxonomy = {
  ...
  scenes: [
    {id: 'scene-kitchen', kind: 'scene' as const, name: '厨房', sortOrder: 10, isActive: true},
  ],
  positiveTags: [
...
describe('VideoTaxonomyManager', () => {
  ...
  it('creates a backend-defined scene', async () => {
    const user = userEvent.setup();
    const onCreate = vi.fn().mockResolvedValue(undefined);
    ...
    await user.type(screen.getByPlaceholderText('新增场景'), '户外');
    await user.click(screen.getByRole('button', {name: '添加'}));

    expect(onCreate).toHaveBeenCalledWith({kind: 'scene', name: '户外'});
  });
});
