...
export const ResumeImportModal = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-surface-muted0/20 backdrop-blur-sm p-4 md:p-10">
      <motion.div
        initial={{opacity: 0, scale: 0.95, y: 10}}
        animate={{opacity: 1, scale: 1, y: 0}}
        className="bg-surface w-full max-w-4xl rounded-2xl shadow-xl flex flex-col max-h-full overflow-hidden"
      >
        <div className="p-6 md:p-8 flex-1 overflow-y-auto">
          <div className="flex justify-between items-start mb-6">
            <div>
              <h2 className="text-2xl font-bold text-fg mb-2">批量导入简历</h2>
              <p className="text-fg-muted text-sm">AI 自动解析·最多100份</p>
            </div>
            <button onClick={handleClose} className="p-2 text-fg-faint hover:text-fg-secondary transition-colors">
              <X className="w-6 h-6" />
            </button>
          </div>
          ...
          <div className="flex items-center space-x-4 mb-8">
            <div className="flex items-center text-[#1a4bc4] font-medium">
              <div className={`w-6 h-6 rounded-full flex items-center justify-center text-sm mr-2 ${step >= 1 ? 'bg-[#1a4bc4] text-white' : 'bg-surface-muted text-fg-faint'}`}>1</div>
              上传文件
            </div>
            <div className="h-px w-16 bg-[#1a4bc4]"></div>
            <div className={`flex items-center font-medium ${step >= 2 ? 'text-[#1a4bc4]' : 'text-fg-faint'}`}>
              <div className={`w-6 h-6 rounded-full flex items-center justify-center text-sm mr-2 ${step >= 2 ? 'bg-[#1a4bc4] text-white' : 'border border-border text-fg-faint'}`}>2</div>
              AI 解析中
            </div>
            <div className={`h-px w-16 ${step >= 2 ? 'bg-[#1a4bc4]' : 'bg-gray-200'}`}></div>
            <div className={`flex items-center font-medium ${step >= 3 ? 'text-[#1a4bc4]' : 'text-fg-faint'}`}>
              <div className={`w-6 h-6 rounded-full flex items-center justify-center text-sm mr-2 ${step >= 3 ? 'bg-[#1a4bc4] text-white' : 'border border-border text-fg-faint'}`}>3</div>
              查看结果
            </div>
          </div>
          ...
          {step === 1 ? (
            <div className="space-y-6">
              <div
                className={`border border-dashed rounded-xl p-10 flex flex-col items-center justify-center text-center transition-colors ${
                  isDragging ? 'border-[#1a4bc4] bg-[#EEF2FF]' : 'border-[#1a4bc4] bg-surface'
                }`}
              ...
              >
                ...
                <div className="text-lg font-bold text-fg mb-1">拖拽 PDF / Word 简历到这里</div>
                <div className="text-fg-muted mb-6 font-normal">
                  或 <button onClick={handleClickSelectFiles} className="text-[#1a4bc4] hover:underline">点击选择文件</button> (最多100份，总大小≤500MB)
                </div>
                <div className="flex space-x-2">
                  <span className="bg-[#E2E8F0] text-fg-secondary px-3 py-1 rounded text-sm font-medium">PDF · DOC · DOCX · PNG · JPG</span>
                </div>
                {/* Hidden file input */}
                <input
                  ref={fileInputRef}
              ...
              <div className="border border-border rounded-xl overflow-hidden">
                <table className="w-full text-left text-sm">
                  <thead className="bg-[#F8FAFC] text-fg-muted text-base border-b border-border">
                    <tr>
                  ...
                  <tbody className="divide-y divide-border-subtle bg-surface">
                    {selectedFiles.length > 0 ? (
                      selectedFiles.map((file, index) => {
                        ...
                        return (
                          <tr key={`${file.name}-${file.size}-${index}`}>
                            <td className="px-6 py-4 text-fg font-medium">{file.name}</td>
                            <td className="px-6 py-4 text-fg">{sizeMB}MB</td>
                      ...
                      <tr>
                        <td colSpan={3} className="px-6 py-8 text-center text-fg-faint">
                          请选择要导入的简历文件
                        </td>
                      </tr>
                    )}
                ...
                {selectedFiles.length > 0 && (
                  <div className="bg-[#F8FAFC] border-t border-border px-6 py-3 text-sm text-fg-secondary">
              ...
              <div className="space-y-5 pt-2">
                <div className="flex items-center">
                  <label className="w-32 text-fg-secondary font-medium">解析后归入项目 :</label>
                  <div className="relative flex-1 max-w-sm">
                    {loadingProjects ? (
                      <div className="w-full border border-border rounded-lg py-2 pl-4 text-fg-muted text-sm">
                        加载中...
                      </div>
                    ) : (
                      <>
                        <select
                          ...
                          className="appearance-none w-full border border-border rounded-lg py-2 pl-4 pr-10 outline-none text-fg bg-surface"
                        >
                ...
                <div className="flex items-center">
                  <label className="w-32 text-fg-secondary font-medium">主要岗位类型 :</label>
                  <div className="relative flex-1 max-w-sm">
                    {loadingProjects ? (
                      <div className="w-full border border-border rounded-lg py-2 pl-4 text-fg-muted text-sm">
                        加载中...
                      ...
                      <>
                        <select
                          ...
                          className="appearance-none w-full border border-border rounded-lg py-2 pl-4 pr-10 outline-none text-fg bg-surface disabled:bg-surface-muted disabled:cursor-not-allowed"
                        >
                ...
                <div className="flex items-center">
                  <label className="w-32 text-fg-secondary font-medium">自动触发 Fit Score 评分 :</label>
                  <button onClick={() => setIsAutoFit(!isAutoFit)} className={`w-11 h-6 rounded-full relative transition-colors ${isAutoFit ? 'bg-[#1a4bc4]' : 'bg-gray-200'}`}>
                    <motion.div layout className="absolute top-1 left-1 bg-surface w-4 h-4 rounded-full shadow-sm" animate={{x: isAutoFit ? 20 : 0}} transition={{type: 'spring', stiffness: 500, damping: 30}} />
                  </button>
                </div>
              </div>
              ...
              <div className="flex items-center space-x-4 pt-4">
                ...
                <button onClick={handleClose} className="px-6 py-3 text-fg-secondary hover:bg-surface-muted rounded-lg font-medium transition-colors">
                  取消
                </button>
              </div>

              <div className="flex items-center space-x-2 text-fg-muted text-sm pb-2">
            ...
            <div className="py-24 flex flex-col items-center justify-center">
              ...
              <div className="text-xl font-bold text-fg mb-2">正在通过 AI 提取并分析简历信息...</div>
              <div className="text-fg-muted">正在处理 {selectedFiles.length} 份简历...</div>
              <div className="w-full max-w-md bg-surface-muted h-2.5 rounded-full mt-8">
            ...
            <div className="py-16 flex flex-col items-center justify-center">
              ...
              <div className="text-xl font-bold text-fg mb-2">
                {importResult && importResult.failed > 0 ? '部分完成' : '解析完成'}
              </div>
              <div className="text-fg-muted mb-2 text-center">
                {importResult
