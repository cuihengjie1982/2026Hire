...
export default function TrainingRecommendations({employeeId}: TrainingRecommendationsProps) {
  ...
  if (loading) return <div className="text-xs text-gray-400 py-2">加载培训推荐...</div>;

  return (
    <div className="mt-3 border-t border-gray-200 pt-3">
      <div className="flex items-center justify-between mb-3">
        <h4 className="text-sm font-semibold text-gray-700">推荐培训</h4>
      ...
      {recs.length === 0 ? (
        ...
        <div className="space-y-2">
          {recs.map(rec => {
            ...
            return (
              <div key={rec.id} className="flex items-center gap-3 bg-gray-50 rounded-lg p-2.5">
                <BookOpen className="w-4 h-4 text-gray-400 flex-shrink-0" />
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-medium text-gray-700 truncate">{rec.courseTitle ?? '未知课程'}</div>
                  <div className="flex items-center gap-2 mt-0.5">
                    <span className={`text-[10px] px-1.5 py-0.5 rounded ${reasonInfo.color}`}>{reasonInfo.label}</span>
                    {rec.reasonDetail && <span className="text-[11px] text-gray-400 truncate">{rec.reasonDetail}</span>}
                  </div>
                </div>
