...
const CompletionOverlay = ({
  score, timeLeft, onClose, onExport, exporting,
}: {
...
const ConversationInterviewPageInner = () => {
  ...
  return (
    <div className="min-h-dvh w-full bg-gradient-to-br from-[#F5F3FF] to-[#EBE0FF] flex flex-col relative" style={{ paddingBottom: 'env(safe-area-inset-bottom, 0px)' }}>
      ...
      <footer className="flex-shrink-0 bg-surface border-t border-border px-2 sm:px-4 py-2.5 sm:py-3">
        <div className="max-w-3xl mx-auto space-y-2 sm:space-y-3">
          ...
          <div className="flex items-end gap-3">
            ...
            {/* Text input */}
            <textarea
              value={voice.isListening ? (input + (voice.interim ? (input ? ' ' : '') + voice.interim : '')) : input}
      ...
      {/* Completion overlay */}
      {state === 'completed' && (
        <CompletionOverlay score={score} timeLeft={timeLeft} onClose={handleClose} onExport={handleExport} exporting={exporting} />
      )}
    </div>
  );
};
