...
// GET /analytics/project-stats — project dashboard stats
...
export const interviewSummary = async (req: Request, _userId: string, _userRole: string): Promise<Response> => {
  try {
    ...
    return jsonRes({ totalInterviews: total, completed, reviewed, passRate, avgScore, avgDuration, gradeDistribution, trends: [] });
  } catch {
    return jsonRes({ error: { code: 'INTERNAL_ERROR', message: 'An internal error occurred' } }, 500);
  }
};
  ...
  try {
    ...
    for (const row of rows) {
      ...
      for (const d of dims) {
        ...
        entry.scores.push(score); entry.count++;
      ...
      for (const a of answers) {
        ...
        entry.scores.push(score); entry.total++;
    ...
    return jsonRes({ dimensions, questions, weakestDimension: weakestDim?.name ?? null, hardestQuestion: hardestQ?.questionTitle ?? null });
  } catch {
    return jsonRes({ error: { code: 'INTERNAL_ERROR', message: 'An internal error occurred' } }, 500);
  }
};
